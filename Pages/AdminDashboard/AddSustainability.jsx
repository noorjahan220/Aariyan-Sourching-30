    ...
    <div className="flex flex-col items-center gap-4 p-4 bg-white shadow-lg rounded-md">
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          <p className="font-semibold text-gray-800">Delete "{sustainability?.value}"?</p>
          ...
          onClick={async () => {
            try {
              const res = await axiosSecure.delete(
                `/delete-productAttribute/sustainability/${sustainability.id}`
              );

              if (res.data.modifiedCount > 0) {
                toast.success("Sustainability deleted successfully.", {
                  duration: 2000,
                });
               
              } else {
                toast.error("Failed to delete Sustainability!");
              }
            } catch (error) {
              console.error(error);
              toast.error(
